...
class ElegirRespuesta_MAT021_iv1(models.Model):
	...
	pregunta = models.ForeignKey(Pregunta_MAT021_iv1, related_name='opciones', on_delete=models.CASCADE)
	correcta = models.BooleanField(verbose_name='¿Es esta la pregunta correcta?', default=False, null=False)
	texto = models.TextField(verbose_name='Texto de la respuesta')
...
class QuizUsuario_MAT021_iv1(models.Model):
	...
	def obtener_nuevas_preguntas(self):
		respondidas = PreguntasRespondidas.objects.filter(quizUser=self).values_list('pregunta__pk', flat=True)
		preguntas_restantes = Pregunta_MAT021_iv1.objects.exclude(pk__in=respondidas)
		if not preguntas_restantes.exists():
			return None
		return random.choice(preguntas_restantes)
...
class PreguntasRespondidas_MAT021_iv1(models.Model):
	quizUser = models.ForeignKey(QuizUsuario_MAT021_iv1, on_delete=models.CASCADE, related_name='intentos')
	pregunta = models.ForeignKey(Pregunta_MAT021_iv1, on_delete=models.CASCADE)
	respuesta = models.ForeignKey(ElegirRespuesta_MAT021_iv1, on_delete=models.CASCADE, null=True)
	correcta  = models.BooleanField(verbose_name='¿Es esta la respuesta correcta?', default=False, null=False)
	puntaje_obtenido = models.DecimalField(verbose_name='Puntaje Obtenido', default=0, decimal_places=2, max_digits=6)